...
use crate::scorer::{AppRiskProfile, TrustResult, compute_trust};
...
pub fn classify_permissions(perms_json: &str) -> Result<Vec<PermissionEntry>, String> {
    let perms: Vec<String> = serde_json::from_str(perms_json)
        .map_err(|e| e.to_string())?;
    let entries = perms.into_iter().map(|p| {
