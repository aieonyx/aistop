...
object ExportManager {
    ...
    suspend fun exportSignedReport(context: Context): ExportResult =
        withContext(Dispatchers.IO) {
            ...
            // EdisonDB may fail on some devices — export with empty log rather than crash
            val events = try {
                ExposureDatabase.getInstance(context).exposureDao().getAllForExport()
            } catch (e: Exception) {
                android.util.Log.w("AiStop", "EdisonDB unavailable for export: ${e.message}")
                emptyList()
            }
    ...
    fun shareExport(context: Context, file: File) {
        ...
        val intent = Intent(Intent.ACTION_SEND).apply {
            ...
            putExtra(Intent.EXTRA_SUBJECT, "AI Stop Exposure Report — Ed25519 Signed")
            putExtra(Intent.EXTRA_TEXT, "Sovereign AI exposure log. Signed with Ed25519 + BLAKE3. Generated by AI Stop.")
            addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION)
            addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
        }
        val chooser = Intent.createChooser(intent, "Share signed exposure report").apply {
            addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
        }
